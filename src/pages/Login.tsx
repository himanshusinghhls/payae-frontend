...
export default function Login() {
  ...
  const [forgotPasswordStep, setForgotPasswordStep] = useState<"EMAIL" | "OTP_PASSWORD">("EMAIL");
  ...
  const loginMutation = useMutation({
    ...
    onError: (error: any) => toast.error(error.response?.data?.message || "Invalid email or password.")
  });
  ...
  const sendOtpMutation = useMutation({
    ...
    onError: (error: any) => toast.error(error.response?.data?.message || "Failed. Email might already be in use.")
  });
  ...
  const forgotPasswordMutation = useMutation({
    mutationFn: async () => await api.post("/auth/forgot-password", { email }),
    onSuccess: () => {
      toast.success("Password reset code sent to email!");
      setForgotPasswordStep("OTP_PASSWORD");
    },
    onError: (error: any) => toast.error(error.response?.data?.message || "Account not found.")
  });

  const resetPasswordMutation = useMutation({
    mutationFn: async () => await api.post("/auth/reset-password", { email, otp, newPassword: password }),
    onSuccess: () => {
    ...
    onError: (error: any) => toast.error(error.response?.data?.message || "Invalid or expired OTP.")
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isForgotPassword) {
      if (forgotPasswordStep === "EMAIL") forgotPasswordMutation.mutate();
      else resetPasswordMutation.mutate();
    } else {
  ...
  const isLoading = loginMutation.isPending || sendOtpMutation.isPending || googleAuthMutation.isPending || forgotPasswordMutation.isPending || resetPasswordMutation.isPending;
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1C] text-white flex flex-col justify-center items-center p-4 relative overflow-hidden selection:bg-payae-accent selection:text-black">
      <Toaster position="top-center" toastOptions={{ style: { background: '#0A0F1C', color: '#fff', borderRadius: '16px', border: '1px solid rgba(0,229,255,0.2)' } }} />
      ...
      <motion.div layout className="w-full max-w-md bg-black/40 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] overflow-hidden z-10">
        <div className="p-8">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={isForgotPassword ? "forgot" : isLogin ? "login" : "register"} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
              
              {isForgotPassword && <h3 className="text-xl font-bold mb-4">Reset Password</h3>}
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                {!isLogin && !isForgotPassword && (
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} required className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white focus:border-payae-accent outline-none transition-all" />
                  </div>
                )}

                {(!isForgotPassword || forgotPasswordStep === "OTP_PASSWORD") && (
                  <>
                    {isForgotPassword && (
                      <input type="text" maxLength={6} placeholder="Enter 6-digit OTP" value={otp} onChange={(e) => setOtp(e.target.value.replace(/[^0-9]/g, ''))} required className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white text-center tracking-[0.5em] font-bold focus:border-payae-accent outline-none transition-all" />
                    )}
                    <div className="relative">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                      <input type="password" placeholder={isForgotPassword ? "New Password" : "Password"} value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white focus:border-payae-accent outline-none transition-all" />
                    </div>
                  </>
                )}
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={isLoading} type="submit" className="w-full mt-6 bg-gradient-to-r from-payae-accent to-blue-600 text-black font-bold py-4 rounded-xl shadow-lg hover:shadow-[0_0_20px_rgba(0,229,255,0.4)] transition-all flex justify-center items-center gap-2 disabled:opacity-70">
                  {isLoading ? <Loader2 className="animate-spin" size={20} /> : <>{isForgotPassword ? (forgotPasswordStep === "EMAIL" ? "Send Code" : "Reset Password") : isLogin ? "Sign In" : "Request OTP"} <ArrowRight size={18} /></>}
                </motion.button>
        ...
        <div className="bg-white/5 p-6 text-center border-t border-white/10">
          <p className="text-gray-400 text-sm">
            ...
            <button type="button" onClick={toggleAuthMode} className="ml-2 text-payae-accent font-bold hover:underline">{isForgotPassword || !isLogin ? "Sign in instead" : "Register here"}</button>
          </p>
        </div>
      </motion.div>
    </div>
