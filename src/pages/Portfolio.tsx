      ...
      <div className="max-w-6xl mx-auto mt-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-4 border-b border-white/10 gap-4">
          ...
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-black/40 backdrop-blur-md px-5 py-2.5 rounded-xl border border-white/10 text-center shadow-lg">
            <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-0.5">Total Liquid Value</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-black/40 backdrop-blur-2xl border border-white/10 p-6 md:p-8 rounded-2xl shadow-2xl mb-8">
            <h3 className="text-lg font-bold text-white mb-5 flex items-center gap-2">
              <PieChart className="text-payae-accent w-5 h-5" /> Distribution Breakdown
            </h3>
            
            <div className="w-full h-8 bg-gray-900 rounded-full flex overflow-hidden shadow-inner border border-white/5 mb-3 relative group cursor-crosshair">
              <motion.div initial={{ width: 0 }} animate={{ width: `${savPct}%` }} transition={{ duration: 1 }} className="h-full bg-[#00E5FF] flex items-center justify-center shadow-[inset_0_0_10px_rgba(0,0,0,0.2)]">
                 {savPct > 10 && <span className="text-black font-bold text-[10px] truncate px-2">Savings</span>}
              </motion.div>
              <motion.div initial={{ width: 0 }} animate={{ width: `${mfPct}%` }} transition={{ duration: 1, delay: 0.2 }} className="h-full bg-[#00FF94] flex items-center justify-center shadow-[inset_0_0_10px_rgba(0,0,0,0.2)]">
                 {mfPct > 10 && <span className="text-black font-bold text-[10px] truncate px-2">Funds</span>}
              </motion.div>
              <motion.div initial={{ width: 0 }} animate={{ width: `${goldPct}%` }} transition={{ duration: 1, delay: 0.4 }} className="h-full bg-[#f58220] flex items-center justify-center shadow-[inset_0_0_10px_rgba(0,0,0,0.2)]">
                 {goldPct > 10 && <span className="text-white font-bold text-[10px] truncate px-2">Gold</span>}
              </motion.div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} whileHover={{ y: -5, scale: 1.02 }} className="bg-gradient-to-br from-[#00E5FF]/10 to-black/40 border border-[#00E5FF]/30 p-5 rounded-2xl flex flex-col justify-between shadow-lg cursor-pointer">
            <div className="flex items-center justify-between mb-6">
              <div className="bg-[#00E5FF]/20 p-2.5 rounded-lg"><Wallet className="text-[#00E5FF] w-5 h-5" /></div>
              <span className="text-[#00E5FF] font-bold text-lg">{savPct.toFixed(1)}%</span>
            </div>
            <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} whileHover={{ y: -5, scale: 1.02 }} className="bg-gradient-to-br from-[#00FF94]/10 to-black/40 border border-[#00FF94]/30 p-5 rounded-2xl flex flex-col justify-between shadow-lg cursor-pointer">
            <div className="flex items-center justify-between mb-6">
              <div className="bg-[#00FF94]/20 p-2.5 rounded-lg"><TrendingUp className="text-[#00FF94] w-5 h-5" /></div>
              <span className="text-[#00FF94] font-bold text-lg">{mfPct.toFixed(1)}%</span>
            </div>
            <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} whileHover={{ y: -5, scale: 1.02 }} className="bg-gradient-to-br from-[#f58220]/10 to-black/40 border border-[#f58220]/30 p-5 rounded-2xl flex flex-col justify-between shadow-lg cursor-pointer">
            <div className="flex items-center justify-between mb-6">
              <div className="bg-[#f58220]/20 p-2.5 rounded-lg"><Coins className="text-[#f58220] w-5 h-5" /></div>
              <span className="text-[#f58220] font-bold text-lg">{goldPct.toFixed(1)}%</span>
            </div>
            <div className="flex justify-between items-end">
              <div>
                <h3 className="text-gray-400 font-bold uppercase tracking-wider text-[11px] mb-1">Digital Gold</h3>
                <p className="text-3xl font-black text-white">₹{goldValueInRupees.toFixed(2)}</p>
              </div>
