...
export default function AllocationChart({ portfolio }: any) {
  ...
  const goldValue = (portfolio?.goldGrams || 0) * 7500; 

  const hasData = savings > 0 || mf > 0 || goldValue > 0;

  const data = {
    labels: ["Liquid Savings", "Mutual Funds", "Digital Gold"],
    datasets: [
      {
        data: hasData ? [savings, mf, goldValue] : [1],
  ...
  const options = {
    ...
    plugins: {
      legend: {
        ...
        labels: { color: "#9ca3af", usePointStyle: true, padding: 20, font: { family: "Inter", size: 12 } },
      },
      tooltip: {
        enabled: hasData,
    ...
    <div className="h-full w-full min-h-[300px] flex flex-col relative">
      <div className="flex-grow relative flex justify-center items-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mb-8">
          <span className="text-gray-400 text-xs uppercase tracking-widest font-semibold">Status</span>
          <span className="text-xl font-black text-white">{hasData ? "Active" : "Pending"}</span>
        </div>
      </div>
    </div>
  );
