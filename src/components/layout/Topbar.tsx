  ...
  return (
    <div className="h-20 backdrop-blur-md bg-payae-bg/80 border-b border-payae-border flex items-center justify-between px-8 sticky top-0 z-30">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400">Here is your wealth overview for today.</p>
      </div>
      ...
      <div className="flex items-center gap-6 relative">
        ...
        <div className="relative">
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-72 bg-payae-card border border-payae-border backdrop-blur-xl rounded-2xl shadow-2xl p-4 z-40">
        ...
        {/* User Avatar */}
        <div className="relative">
          <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-10 h-10 rounded-full bg-gradient-to-tr from-payae-accent to-blue-600 flex items-center justify-center shadow-lg border border-white/10">
          ...
          <AnimatePresence>
            {showProfileMenu && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-56 bg-payae-card border border-payae-border backdrop-blur-xl rounded-2xl shadow-2xl p-2 z-40 overflow-hidden">
                ...
                <button onClick={() => { logout(); window.location.href = '/login'; }} className="w-full flex items-center gap-3 px-3 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-colors mt-1">
